...
The production bridge decision is recorded in [Flow Coach bridge architecture](flow-coach-bridge-architecture.md). It keeps the same contract behind a primary provider-neutral MCP edge, the Flow device edge, and a fallback Actions adapter. Each Flow installation pairs with one isolated single-person mailbox deployment; multiple people do not share bridge credentials or remote data. Claude has proved the MCP loop on desktop and iOS; #49 proved ChatGPT compatibility from ChatGPT Work and closed there, because Work has no access to a person's ordinary ChatGPT memory and connectors do not reach ordinary Chat or mobile.
...
The ceilings themselves live in `bridge-worker/src/patch-operations.json` alongside the operation kinds, and are shared the same way: the bridge derives its schemas from the file, and `FlowTests` asserts the app's `FlowTextBounds` constants against it. The app enforces the same bounds at every point of entry — the routine and exercise editors, whole-routine JSON import, the constraints notes field — at the point of typing, so a value the app accepts is never one the snapshot upload refuses.

The snapshot itself relays names exactly as the app stores them. The snapshot schema used to trim names on the way in, which meant the coach could only ever see `"Bench"` for a device holding `"Bench "`; any `expectedStringValue` copied out of the snapshot then failed on the phone as `beforeValueMismatch`, with the value the coach actually needed being one the snapshot never showed it. Now the bridge stores the app's bytes, refuses a blank name, and measures the bound on the trimmed form — the same measurement the app's editors make, so a value the app accepts is never one the upload refuses, including a legacy name whose invisible padding pushes the raw bytes past the ceiling. A padded name can only be legacy data — the app trims on every write — and relaying it exactly is what lets the coach anchor operations to it.
...
Two differences are left standing, deliberately, and in both the refusal happens rather than something wrong being accepted. Swift compares strings under Unicode canonical equivalence and the bridge compares code unit for code unit, so an `expectedStringValue` written in a decomposed form conflicts at the bridge where it would have matched on the device. And the app trims a strict superset of what JavaScript's `trim` strips — `whitespacesAndNewlines` plus U+FEFF, where JavaScript takes neither U+0085 nor U+200B — so a value the coach proposes with exotic padding can be stored by the bridge and still land trimmed on the device. In both cases the fix is to copy the value out of the snapshot rather than retyping it.
...
Expected before-values follow the same rule. Every operation that carries one is checked against the value the routine actually holds, not merely for the field being present: `expectedIntValue` for the numeric replaces, `expectedStringValue` for `updateExerciseNotes`, `removeExercise` and `renameRoutine`, and `expectedPhaseOverride` for `replacePhaseOverride`. A mismatch is a conflict and is refused, because Flow refuses it too. Where an earlier operation in the same patch changed the field, the expected value is the one that operation left behind, not the one in the snapshot. `expectedPhaseOverride` is compared exactly, and omitting it is not the same as sending an empty object: absent means the exercise has no override for that phase, and Flow keeps the two apart.
